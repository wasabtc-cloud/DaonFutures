...
private fun oneCandleZone(candles: List<Candle>, e20: List<Double>, e50: List<Double>): OneCandleZone? {
    ...
    val key = (if (bullish) indexed.filter { (_, c) -> c.close < c.open }.maxByOrNull { (_, c) -> c.high }
    else indexed.filter { (_, c) -> c.close > c.open }.minByOrNull { (_, c) -> c.low }) ?: return null
    val (idx, candle) = key
    ...
    if (!confirmed) return OneCandleZone(idx, low, high, side, retested, false, null, null, null)
    val entry = last.close
    val stop = if (bullish) low else high
    val risk = if (bullish) entry - stop else stop - entry
    if (risk <= 0.0) return OneCandleZone(idx, low, high, side, retested, false, null, null, null)
...
@Composable
fun MarketChartCard(symbol: String, intervalLabel: String, candles: List<Candle>, modifier: Modifier = Modifier) {
    ...
    val flowMarks = moneyFlowMarks(visible)
    val superSignal = flowMarks.lastOrNull()?.level == 3 && zone?.confirmed == true
    ...
    Card(modifier = modifier.fillMaxWidth(), shape = RoundedCornerShape(18.dp)) {
        Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            if (visible.size < 2) Text("차트 데이터를 불러오는 중입니다.", color = Color.Gray) else {
                ...
                Text("자금유입 표시 ON · ✓ 초기  ✓✓ 추가  ★ 강한 유입", color = Color(0xFF21D58B), fontWeight = FontWeight.Bold, fontSize = 12.sp)
                if (superSignal) Text("S  SUPER SIGNAL · 강한 자금유입 + 진입조건 확인", color = Color(0xFFFFC107), fontWeight = FontWeight.ExtraBold, fontSize = 14.sp)
                zone?.let {
                    val col = if (it.side == "LONG") Color(0xFF21D58B) else Color(0xFFFF4058)
                    val state = when { it.confirmed -> "진입 확정"; it.retested -> "리테스트 확인 · 방향 확인 대기"; else -> "리테스트 대기" }
                    Text("One Candle ${it.side} · 기준 ${fmtChart(it.low)} ~ ${fmtChart(it.high)} · $state", color = col, fontWeight = FontWeight.Bold, fontSize = 12.sp)
                } ?: Text("One Candle · 기준 캔들 탐색 중", color = Color.Gray, fontSize = 12.sp)
                CandlestickCanvas(visible, e20, e50, zone, flowMarks)
                zone?.takeIf { it.confirmed }?.let {
                    Text("진입 ${fmtChart(it.entry)} · SL ${fmtChart(it.stop)} · 2R ${fmtChart(it.target2R)}", color = if (it.side == "LONG") Color(0xFF21D58B) else Color(0xFFFF4058), fontWeight = FontWeight.Bold, fontSize = 12.sp)
                }
...
@Composable
private fun CandlestickCanvas(candles: List<Candle>, e20: List<Double>, e50: List<Double>, zone: OneCandleZone?, flowMarks: List<MoneyFlowMark>) {
    ...
    Canvas(Modifier.fillMaxWidth().height(360.dp).background(Color(0xFF090E16), RoundedCornerShape(12.dp))) {
        ...
        zone?.let { extras += it.low; extras += it.high; it.stop?.let { v -> extras += v }; it.target2R?.let { v -> extras += v } }
        val candleMax = candles.maxOf { it.high }; val candleMin = candles.minOf { it.low }
        val maxP = max(candleMax, extras.maxOrNull() ?: candleMax); val minP = min(candleMin, extras.minOrNull() ?: candleMin)
        val rawRange = maxP - minP; val range = if (rawRange <= 0.0) 1.0 else rawRange; val pad = range * 0.12
        val top = maxP + pad; val bottom = minP - pad; val full = top - bottom
        ...
        val step = plotWidth / candles.size; val body = max(2f, step * 0.58f)
        zone?.let {
            val zoneColor = if (it.side == "LONG") up else down; val left = (step * it.index).coerceAtLeast(0f)
            val topY = min(y(it.high), y(it.low)); val bottomY = max(y(it.high), y(it.low))
            drawRect(zoneColor.copy(alpha = 0.20f), Offset(left, topY), Size(plotWidth - left, max(3f, bottomY - topY)))
            drawLine(zoneColor, Offset(left, topY), Offset(plotWidth, topY), 2f); drawLine(zoneColor, Offset(left, bottomY), Offset(plotWidth, bottomY), 2f)
        }
        candles.forEachIndexed { i, candle ->
            val x = step * i + step / 2f; val candleColor = if (candle.close >= candle.open) up else down
        ...
        flowMarks.forEach { mark ->
            ...
            val markerColor = if (mark.level == 3) Color(0xFFFFC107) else up
            val radius = when(mark.level) { 3 -> 8f; 2 -> 6f; else -> 4.5f }
            drawCircle(markerColor, radius = radius, center = Offset(x, markerY))
