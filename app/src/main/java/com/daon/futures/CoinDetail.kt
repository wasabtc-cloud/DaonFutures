...
@Composable fun CoinDetailScreen(asset:String="BTC"){
 ...
 LazyColumn(Modifier.fillMaxSize().padding(horizontal=14.dp),contentPadding=PaddingValues(vertical=14.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){item{Text(asset.uppercase(),style=MaterialTheme.typography.headlineMedium)};item{Row(horizontalArrangement=Arrangement.spacedBy(6.dp)){listOf("UPBIT","BINANCE","BYBIT").forEach{FilterChip(selected=exchange==it,onClick={exchange=it},label={Text(it)})}}};item{Row(horizontalArrangement=Arrangement.spacedBy(6.dp)){listOf("5분","15분","1시간","4시간").forEach{FilterChip(selected=interval==it,onClick={interval=it},label={Text(it)})}}};item{quote?.let{Text("${it.exchange}  ${it.price}  24H ${"%.2f".format(it.changePct)}%") }?:Text(if(loading)"실제 시장 데이터를 불러오는 중…" else "데이터를 불러오지 못했습니다.")};item{if(loading)LinearProgressIndicator(Modifier.fillMaxWidth()) else MarketChartCard("${asset.uppercase()} · $exchange",interval,candles,Modifier.fillMaxWidth())};item{Text("✓ / ✓✓ / S는 연구·검증 중인 신호이며 자동주문은 실행하지 않습니다.",style=MaterialTheme.typography.bodySmall)}}
}