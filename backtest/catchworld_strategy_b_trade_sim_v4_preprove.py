...
OUT='results/strategy_b_trade_sim_v4_preprove';os.makedirs(OUT,exist_ok=True)
...
# V4: WATCH -> READY -> IGNITION -> PRE-BUY PROVE -> BUY. No capital is committed before proof.
sigs=[]
for m,g in d.groupby('market',sort=False):
 ...
 for j in np.flatnonzero(watch.to_numpy()):
  ...
  ignite=hit[0];entry0=float(g.loc[ignite].close)
  # PRE-BUY PROVE: inspect hours 3-6 after ignition, but buy only when proof actually appears.
  pw=g.loc[(g.index>=ignite+pd.Timedelta(hours=3))&(g.index<=ignite+pd.Timedelta(hours=6))]
  prove_t=None
  for t,r in pw.iterrows():
   recent=g.loc[(g.index>=ignite)&(g.index<=t)].tail(3);flow_med=recent.turn_ratio.median();price_ok=float(r.close)>=entry0*.995;structure=(r.ret_6h>0) or (float(r.close)>=recent.close.min()*1.01)
   if flow_med>=1.8 and r.turn_ratio>=1.5 and price_ok and structure:prove_t=t;break
  if prove_t is not None:
   sigs.append((prove_t,m,wt,ignite,setup));cool=prove_t+pd.Timedelta(hours=72)
s=pd.DataFrame(sigs,columns=['buy_time','market','watch_time','ignite_time','setup']).sort_values('buy_time');print('PREPROVED_BUY_SIGNALS',len(s))
...
for z in s.itertuples(index=False):
 ...
 if len(w)<2:continue
 entry=float(w.close.iloc[0]);peak=entry;exit_t=w.index[-1];exit_p=float(w.close.iloc[-1]);reason='TIME72';weakst=0
 ...
 net=(exit_p/entry)*(1-FEE)/(1+FEE);before=equity;equity*=net;tr.append({'market':z.market,'setup':z.setup,'watch_time':z.watch_time,'ignite_time':z.ignite_time,'buy_time':z.buy_time,'sell_time':exit_t,'entry':entry,'exit':exit_p,'net_ret':net-1,'hold_h':(exit_t-z.buy_time).total_seconds()/3600,'reason':reason,'equity_before':before,'equity_after':equity});busy=exit_t
...
if len(r):
 ...
 summary={'start_krw':START,'final_krw':equity,'multiple':equity/START,'trades':len(r),'win_rate':(r.net_ret>0).mean(),'avg_net_ret':r.net_ret.mean(),'median_net_ret':r.net_ret.median(),'pf':pf,'mdd':mdd,'avg_hold_h':r.hold_h.mean(),**{x:int((r.reason==x).sum()) for x in ['HARD7','DEAD12','FLOW_DD8','WEAK36','TIME72']}}
else:summary={'start_krw':START,'final_krw':START,'multiple':1,'trades':0}
pd.DataFrame([summary]).to_csv(OUT+'/summary.csv',index=False);print(pd.DataFrame([summary]).to_string(index=False));print(r.groupby(['setup','reason']).net_ret.agg(['count','mean','median']).to_string() if len(r) else '')
