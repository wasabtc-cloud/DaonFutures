"""Fair SWING5/10/20 comparison. Only initial stop lookback differs."""
import pandas as pd
import numpy as np
...
def add_swings(raw,start):
    ...
    for n in SWINGS:d[f'swing{n}']=raw.low.shift(1).rolling(n).min().reindex(d.index)
...
def main():
    end=pd.Timestamp(datetime.now(timezone.utc)).floor('D');markets=[x['market'] for x in base.get('/market/all',{'is_details':'false'}) if x['market'].startswith('KRW-')]
    cols,stats,w,th,train_cut=base.train_model(markets,end);valid_start=max(train_cut+pd.Timedelta(days=1),end-pd.Timedelta(days=base.VALID_DAYS));universe=base.market_universe(markets,valid_start);buckets={n:[] for n in SWINGS}
    for ix,m in enumerate(universe,1):
        print(f'COMPARE {ix}/{len(universe)} {m}',flush=True);raw=base.fetch_continuous_minutes(m,valid_start-pd.Timedelta(minutes=150),end);d=add_swings(raw,valid_start)
        if d.empty:continue
        ...
        while i<len(d)-2:
            ...
            while j<len(d) and idx[j]<=limit:
                row=d.iloc[j]
                if row.value5_ratio>=base.GREEN_VALUE5 and row.value_accel5>=base.GREEN_ACCEL5 and row.ret5>0 and row.ret15>0:green=j;break
                j+=1
            if green is None:i=j+1;continue
            if green+1>=len(d):break
            entry_time=idx[green+1];entry=float(d.open.iloc[green+1])*(1+base.SLIP);path=raw.loc[(raw.index>=entry_time)&(raw.index<=entry_time+pd.Timedelta(minutes=base.HORIZON_MINUTES))]
            for n in SWINGS:
    ...
    for n in SWINGS:
        ...
        if not tr.empty:tr=tr.sort_values('entry_time').reset_index(drop=True);tr.to_csv(f'natural_swing{n}_compare_trades.csv',index=False);alltr.append(tr)
        summaries.append(summarize(tr,n))
    out=pd.DataFrame(summaries);out.to_csv('natural_swing_compare_summary.csv',index=False)
    if alltr:
        z=pd.concat(alltr,ignore_index=True);z['month']=pd.to_datetime(z.entry_time,utc=True).dt.to_period('M').astype(str);z.groupby(['swing','month']).agg(trades=('result_r','size'),avg_r=('result_r','mean'),sum_r=('result_r','sum'),win_rate_pct=('result_r',lambda x:(x>0).mean()*100)).reset_index().to_csv('natural_swing_compare_monthly.csv',index=False)
