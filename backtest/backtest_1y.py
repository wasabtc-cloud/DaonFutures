...
def st(d,n=10,k=3):
 ...
 for i in range(1,len(d)):
  fu.iloc[i]=ub.iloc[i] if ub.iloc[i]<fu.iloc[i-1] or d.close.iloc[i-1]>fu.iloc[i-1] else fu.iloc[i-1];fl.iloc[i]=lb.iloc[i] if lb.iloc[i]>fl.iloc[i-1] or d.close.iloc[i-1]<fl.iloc[i-1] else fl.iloc[i-1];t.iloc[i]=1 if d.close.iloc[i]>fu.iloc[i-1] else (-1 if d.close.iloc[i]<fl.iloc[i-1] else t.iloc[i-1])
 return t
def adx(d,n=14):
...
def prep(d):
 d=d.copy();d['e20']=ema(d.close,20);d['e50']=ema(d.close,50);d['rsi']=rsi(d.close);d['vma']=d.volume.rolling(20).mean();d['dhi']=d.high.shift().rolling(20).max();d['dlo']=d.low.shift().rolling(20).min()
...
def donchian(d):
 v=d.volume>d.vma;lo=(d.close>d.h4e200)&(d.h1adx>=25)&(d.h1pdi>d.h1mdi)&(d.close>d.dhi)&v;sh=(d.close<d.h4e200)&(d.h1adx>=25)&(d.h1mdi>d.h1pdi)&(d.close<d.dlo)&v;return pd.Series(np.where(lo,1,np.where(sh,-1,0)),index=d.index)
def bosfvg(d):
 ...
 for i in range(2,len(d)):
  if bu.iloc[i] and bf.iloc[i] and d.close.iloc[i]>d.h4e200.iloc[i]:p=('L',d.high.iloc[i-2],d.low.iloc[i]);ex=i+36
  elif bd.iloc[i] and sf.iloc[i] and d.close.iloc[i]<d.h4e200.iloc[i]:p=('S',d.high.iloc[i],d.low.iloc[i-2]);ex=i+36
  if p and i<=ex:
   s,l,h=p
...
def retest(d):
 ...
 for i in range(25,len(d)):
  if d.close.iloc[i]>hi.iloc[i] and d.close.iloc[i]>d.h4e200.iloc[i]:p=('L',hi.iloc[i]);ex=i+24
  elif d.close.iloc[i]<lo.iloc[i] and d.close.iloc[i]<d.h4e200.iloc[i]:p=('S',lo.iloc[i]);ex=i+24
  if p and i<=ex:
   s,z=p
...
def main():
 ss=[('Current app',current),('Supertrend',supertrend),('ADX Donchian',donchian),('BOS FVG',bosfvg),('Breakout Retest',retest)];rows=[];fig,ax=plt.subplots(2,1,figsize=(12,12))
 for a,sym in zip(ax,SYMBOLS):
  ...
  a.axhline(100,ls='--');a.set_title(sym+' 5m 1Y');a.legend();a.grid(alpha=.2)
