...
def signals(d,filters):
 ...
 if 'ADX' in filters:
  L=L&(d.h1adx>=30);S=S&(d.h1adx>=30)
 if 'SLOPE' in filters:
  L=L&(d.h4slope>=.001);S=S&(d.h4slope<=-.001)
 if 'GAP' in filters:
  L=L&(d.emagap>=.002);S=S&(d.emagap>=.002)
 if 'ATR' in filters:
  L=L&(d.atrp>=.003);S=S&(d.atrp>=.003)
...
def main():
 print('Downloading ETHUSDT 15m',flush=True);d=prep(get());d=d.loc[(d.index>=START)&(d.index<END)]
 years=[('Y1','2023-09-10','2024-09-10'),('Y2','2024-09-10','2025-09-10'),('Y3','2025-09-10','2026-09-10')]
 names=['ADX','SLOPE','GAP','ATR'];combos=[()] + list(itertools.combinations(names,2)) + list(itertools.combinations(names,3)) + [tuple(names)]
 rows=[]
 for fs in combos:
  s=signals(d,set(fs));label='BASE' if not fs else '+'.join(fs)
  for mode in ['LONG','SHORT','BOTH']:
   ...
   rows.append([label,mode,round(f[0],2),round(f[0]-100,2),f[1],round(f[2],2),round(f[3],2),round(f[4],2),*[round(x,2) for x in ys]])
 out=pd.DataFrame(rows,columns=['Filters','Mode','Final3Y','Return3Y_pct','Trades','WinRate_pct','PF','MDD_pct','Y1_Final','Y2_Final','Y3_Final']);out=out.sort_values(['Mode','Final3Y'],ascending=[True,False]);print(out.to_string(index=False),flush=True);out.to_csv('backtest_results.csv',index=False)
 print('\nTOP BOTH:',flush=True);print(out[out.Mode=='BOTH'].sort_values('Final3Y',ascending=False).to_string(index=False),flush=True)
